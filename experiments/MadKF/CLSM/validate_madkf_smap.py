...
def run_ascat_eval(iteration, n_procs=1):
    ...
    res_path = Path(f'~/Documents/work/MadKF/CLSM/SMOS40/validation/iter_{iteration}').expanduser()
...
def plot_eval(iteration):
    ...
    res1 = pd.read_csv(Path(f'~/Documents/work/MadKF/CLSM/SMAP/validation/iter_3/ascat_eval.csv').expanduser(), index_col=0)
    res2 = pd.read_csv(Path(f'~/Documents/work/MadKF/CLSM/SMOS40/validation/iter_6/ascat_eval.csv').expanduser(), index_col=0)
    ...
    n_subits = 4
    # exps = ['SMOSSMAP_short'] + [f'SMAP_it{iteration}{i}' for i in range(1, n_subits + 1)] + ['SMOS40_it631']
    exps = ['SMOSSMAP_short', 'SMAP_it34', 'SMOS40_it614', 'SMOS40_it615', 'SMOS40_it613']
    # exps = [f'SMAP_it{iteration}{i}' for i in range(1, n_subits + 1)]
        ...
        for i, m in enumerate(modes):
            for j, e in enumerate(exps):
                ...
                if relative and (p != 'len'):
                    ref_col = f'{sub}r_{ref_exp}_{m}'
                    if col in res1:
                        res1[f'{col}_diff'] = (res1[col] - res1[ref_col]) / (1 - res1[ref_col])
                    else:
                        res1[f'{col}_diff'] = (res2[col] - res1[ref_col]) / (1 - res1[ref_col])
                ...
                im = plot_ease_img(res1, f'{col}{ext}' , title=f'{e} / {m}', cmap=cmap, cbrange=cbr, fontsize=fontsize, print_median=True, log_scale=log_scale)
                # if (i == 2) & (j == 1):
                #     ax = im.axes
...
def plot_centered_cbar(f, im, n_cols, wspace=0.04, hspace=0.025, bottom=0.06, fontsize=12):

    f.subplots_adjust(wspace=wspace, hspace=hspace, bottom=bottom)

    ctr = n_cols/2 * -1
    if ctr % 1 == 0:
        pos1 = f.axes[int(ctr) - 1].get_position()
        pos2 = f.axes[int(ctr)].get_position()
