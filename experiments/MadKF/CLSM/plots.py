...
def plot_ismn_statistics():
    ...
    res = pd.read_csv('/work/MadKF/CLSM/validation/insitu_TCA.csv')
    res.index = res.network
    res2 = pd.read_csv('/work/MadKF/CLSM/validation/insitu.csv')
    ...
    for i_net in np.arange(2):
        ...
        for var, var_label in zip(variables, var_labels):
            ...
            fname = '/work/MadKF/CLSM/validation/plots/' + title + '_' + var + '.png'
            plt.savefig(fname)
            plt.close()
